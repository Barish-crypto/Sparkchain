...
class WebSocketBot {
    ...
    handleMessage(ws, data, token, deviceId) {
        ...
        if (message.startsWith('0')) {
            try {
                ...
                if (!handshake || !handshake.sid) {
                    console.error(chalk.red(`Invalid handshake received for ${token.substring(0, 15)}...`));
                    return;
                }
                ...
                console.error(chalk.red(`Error parsing handshake for ${token.substring(0, 15)}...:`, error.message));
            }
    ...
    reconnect(device, token, attempt = 1) {
        ...
        const retryDelay = Math.min(5000 * attempt, 300000);
